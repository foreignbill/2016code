...
int main(){
	...
	getmin();getmax();
	LL ans=0;
	ap[1]=minsum(2,n,a[1]);aq[n]=maxsum(1,n-1,a[n]);
	couples[1]+=ap[1];couples[n]+=aq[n];
	for(LL i=2;i<=n;i++){
		if(a[i-1]>=a[i]) ap[i]=ap[i-1];
		else ap[i] = maxsum(1,i-1,a[i]);
		couples[i]+=ap[i];
	}
	for(LL i=n-1;i>=1;i--){
		if(a[i]>a[i+1]) aq[i]=aq[i+1];
		else aq[i] = minsum(i+1,n,a[i]);
		couples[i]+=aq[i];
	}
	for(LL i=1;i<=n;i++){
		LL t=ap[i]+aq[i];
